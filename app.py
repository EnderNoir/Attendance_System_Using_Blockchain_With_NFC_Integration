        ...
        rows.append(
            {
                ...
                'section_key': ", ".join(sorted({str(sk).replace('|', ' ') for sk in all_keys if sk})) or 'No Sections',
                ...
                'semester': semester_label or (", ".join(sorted({str(s.get('semester', '')) for s in section_data if s.get('semester')})) or 'Any Semester'),
                'teachers_involved': teachers_involved,
                'programs_involved': programs_involved,
...
def _event_schedule_rows_for_teacher(username):
    ...
    for ev in db_get_all_event_schedules():
        ...
        rows.append(
            {
                'schedule_id': schedule_id,
                'section_key': ", ".join(sorted({str(sk).replace('|', ' ') for sk in section_keys if sk})) or 'No Sections',
...
def _finalize_session(sess_id, ended_time=None, async_chain_and_email=True):
    ...
    involved_sections = {section_key}
    
    if is_school_event:
        # Fetch all sections involved in this event
        sched_id = sess.get('schedule_id')
        if sched_id:
            sched_meta = _parse_event_schedule_id(sched_id)
            ev_id = sched_meta.get('event_id')
            if ev_id:
                ev_row = db_get_event_schedule_by_id(ev_id)
                if ev_row:
                    import json
                    raw_sk = ev_row.get('section_keys_json')
                    if raw_sk:
                        try:
                            s_keys = json.loads(raw_sk)
                            for entry in s_keys:
                                k = entry.get('key') if isinstance(entry, dict) else entry
    ...
    for s in all_students_list:
        sk = build_student_section_key(s)
        if sk in involved_sections:
            if not is_school_event:
                # Classroom session: must match semester if specified
                if sess_semester and normalize_semester(s.get('semester')) and normalize_semester(s.get('semester')) != sess_semester:
                    continue
    ...
    if tx_hash:
        with get_db() as conn:
            if str(sess.get('class_type')).lower() == 'school_event' and sess.get('schedule_id'):
                conn.execute(
                    "UPDATE sessions SET session_tx_hash=?, session_block_number=? WHERE schedule_id=?",
                    (tx_hash, block_num or 0, str(sess.get('schedule_id')))
                )
                # Sync logs for all sessions of this event
                conn.execute(
                    "UPDATE attendance_logs SET tx_hash=?, block_number=? WHERE sess_id IN (SELECT sess_id FROM sessions WHERE schedule_id=?)",
                    (tx_hash, block_num or 0, str(sess.get('schedule_id')))
                )
            else:
                conn.execute(
    ...
    def _post_finalize_worker():
        with app.app_context():
            # For school events, gather all teachers and sections for the email
            is_school_event = str(sess.get('class_type')).lower() == 'school_event'
            teachers_list = None
            sections_list = None
            if is_school_event:
                teachers_list = [sess.get('teacher_name', 'Teacher')]
                sections_list = []
                sched_id = sess.get('schedule_id')
                if sched_id:
                    with get_db() as _conn:
            ...
            for st in section_students:
                ...
                try:
                    ...
                    send_student_attendance_receipt(
                        ...
                        teachers_involved=teachers_list,
    ...
    if preferred:
        with get_db() as conn:
            ...
            if pref_row:
                ...
                # School events: allow any student whose section is involved in the event
                if pref_class_type == 'school_event':
                    # Parse event_id from schedule_id (format: event:<ev_id>:<date>:<sec>)
                    meta = _parse_event_schedule_id(pref_dict.get('schedule_id', ''))
                    if meta and meta.get('event_id'):
                        ev_id = meta['event_id']
                        # Check if student_key is in the event's section list
                        ev = db_get_event_schedule_by_id(ev_id)
                        if ev:
                            import json
                            raw_sk = ev.get('section_keys_json')
                            if raw_sk:
                                try:
                                    involved_sections = json.loads(raw_sk)
                                    # involved_sections is a list of strings or dicts {key, semester}
                                    match_found = False
                                    for entry in involved_sections:
                                        k = entry.get('key') if isinstance(entry, dict) else entry
...
def api_session_attendance(sess_id):
    try:
        ...
        if is_school_event:
            ...
            for rid in related_ids:
                ...
                if rs:
                    ...
                    if sk:
                        section_keys.add(sk)
                        if sem:
                            if sk not in sk_sem_map: sk_sem_map[sk] = set()
                            sk_sem_map[sk].add(sem)
            ...
            if sk:
                if sem: section_display_list.append(f"{sk}|{sem}")
                else: section_display_list.append(sk)
        ...
        if not sess.get('ended_at') or is_school_event:
            all_students = get_all_students()
            sess_semester = normalize_semester(sess.get('semester') or '')
            if is_school_event:
        ...
        return jsonify({
            ...
            'class_type':   sess.get('class_type', 'lecture'),
            'section_key':  section_key,
            'sections_involved': section_display_list or sorted([sk for sk in section_keys if sk]),
            'teachers_involved': teachers_involved,
...
def live_session(sess_id):
    ...
    tapped_nfc_ids = present_set | late_set | excused_set
    existing_nfc_ids = {s['nfcId'] for s in section_students}
    for s in all_students:
        if s['nfcId'] in tapped_nfc_ids and s['nfcId'] not in existing_nfc_ids:
            # Include if they are irregular or managed to tap in
            section_students.append(s)
            existing_nfc_ids.add(s['nfcId'])
    ...
    if is_school_event:
        ...
        if sched_meta:
            ...
            if ev:
                ...
                # Build section_keys with semesters for display
                section_keys = []
                for s in ev.get('section_keys', []):
                    if isinstance(s, dict):
                        k = s.get('key', '')
                        sem = s.get('semester', '')
                        if k:
                            disp = normalize_section_key(k)
                            if sem: disp += f"|{sem}"
                            section_keys.append(disp)
                    elif isinstance(s, str) and s.strip():
                        section_keys.append(normalize_section_key(s))
