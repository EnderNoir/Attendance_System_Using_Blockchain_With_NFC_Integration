/* ΓöÇΓöÇ Build a flat sessions lookup from Jinja ΓöÇΓöÇ */
  const sessionsData = {
  ...
  /* ΓöÇΓöÇ Readable date formatter ΓöÇΓöÇ */
  function fmtReadable(dtStr) {
    if (!dtStr || dtStr === 'None') return 'ΓÇö';
    try {
  ...
  /* ΓöÇΓöÇ Modal inner tab switching ΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ Open session modal ΓöÇΓöÇ */
  function openSessModal(sessId) {
    ...
    document.getElementById('sm_badge').innerHTML = s.ended_at
      ...
      : '<span style="background:rgba(45,106,39,.12);color:var(--success);border:1px solid rgba(45,106,39,.25);border-radius:20px;padding:2px 10px;font-size:10px;font-weight:700;">ΓùÅ LIVE</span>';
    ...
    document.getElementById('sm_att_list').innerHTML =
      ...
      '<div style="margin-top:8px;">Loading attendanceΓÇª</div></div>';
  ...
  function renderSessModal(sessId, data) {
    ...
    <div class="sm-info-box">
      ...
      <div class="sm-info-val">${s.teacher_name || 'ΓÇö'}</div>
    </div>
    <div class="sm-info-box">
      <div class="sm-info-lbl"><i class="bi bi-clock"></i> Time Slot</div>
      <div class="sm-info-val">${s.time_slot || 'ΓÇö'}</div>
    </div>
    <div class="sm-info-box">
    ...
    /* ΓöÇΓöÇ Attendance tab: shows ALL students including absent ΓöÇΓöÇ */
    const stCls = { present: 'st-present', late: 'st-late', absent: 'st-absent', excused: 'st-excused' };
    const stLbl = { present: 'Γ£ô Present', late: 'ΓÅ▒ Late', absent: 'Γ£ò Absent', excused: 'ΓùÄ Excused' };
    ...
    <table class="att-table" style="margin-top:4px;">
      ...
      <tbody>
        ${sts.map((st, i) => {
      const status = (st.status || 'absent').toLowerCase();
      const reasonLabel = st.reason ? (REASON_LABELS[st.reason] || st.reason) : 'ΓÇö';
      ...
      const docLink = st.attachment_url
        ...
        : '<span style="color:var(--muted);font-size:11px;">ΓÇö</span>';
      return `<tr>
            <td class="att-num">${i + 1}</td>
            <td style="font-weight:600;">${st.name || 'ΓÇö'}</td>
            <td style="font-family:'Space Mono',monospace;font-size:11px;color:var(--muted);">${st.student_id || st.nfc_id || 'ΓÇö'}</td>
            <td><span class="att-status ${stCls[status] || 'st-absent'}">${stLbl[status] || status}</span></td>
            <td style="font-family:'Space Mono',monospace;font-size:11px;color:var(--muted);">${st.time || 'ΓÇö'}</td>
            <td style="font-size:11px;">${status === 'excused' ? `<span style="color:#60a5fa;font-weight:600;">${reasonLabel}</span>${reasonDetail}` : '<span style="color:var(--muted);">ΓÇö</span>'}</td>
            <td>${status === 'excused' ? docLink : '<span style="color:var(--muted);font-size:11px;">ΓÇö</span>'}</td>
            <td>${st.tx_hash
          ? `<span class="att-tx">${st.tx_hash.substring(0, 18)}ΓÇª</span>`
          : '<span style="color:var(--muted);font-size:11px;">ΓÇö</span>'}</td>
          </tr>`;
    }).join('')}
      </tbody>
    </table>`;
  ...
  /* ΓöÇΓöÇ Ended filter (multi-filter) ΓöÇΓöÇ */
